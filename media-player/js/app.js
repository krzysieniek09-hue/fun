...
(() => {
  ...
  // The entrance stagger is replayed only when the library's contents
  // change, so routine redraws (a duration arriving, a highlight moving)
  // don't make the whole grid flicker.
  let lastLibrarySig = null;
  let staggerThisRender = false;

  function refreshStaggerFlag() {
    const sig = state.tracks.map((t) => t.id).join("|");
    staggerThisRender = sig !== lastLibrarySig;
    lastLibrarySig = sig;
  }

  function stagger(container) {
    container.classList.toggle("stagger", staggerThisRender);
  }
  ...
  function updateNowPlaying() {
    ...
    const changed = art.dataset.trackId !== t.id;
    art.dataset.trackId = t.id;
    ...
    if (changed) replay([art, document.querySelector(".np-meta")], "pw-swap");
  }
